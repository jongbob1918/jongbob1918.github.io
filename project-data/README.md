...
대표 미디어가 필요 없으면 `demo` 항목 전체를 삭제합니다. `demo.caption`을 지정하면 미디어 아래에 캡션을 표시합니다. 이미지에 `demo.href`를 지정하면 클릭 시 해당 링크를 새 탭으로 엽니다. 본문 중간에 이미지를 추가할 때는 일반 Markdown 이미지 문법을 사용합니다.
