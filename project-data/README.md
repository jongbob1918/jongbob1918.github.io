...
- `collapsibleSections: true`: 상세 페이지의 각 `##` 절을 접힌 상태로 표시합니다. 제목을 누르면 개별적으로 펼치고 접을 수 있으며, 한국어 파일에서 설정하면 두 언어에 적용됩니다.
...
카드와 상세 페이지를 모두 비활성화하고 원본을 보관하려면 한국어 MD를 `archive/`, 영어 MD를 `archive/en/`으로 옮깁니다. 이 폴더는 빌드 대상이 아닙니다. 다시 공개하려면 각각 원래 위치로 옮긴 뒤 빌드합니다.
...
대표 미디어가 필요 없으면 `demo` 항목 전체를 삭제합니다. 그림·영상 박스 하단에 캡션이나 설명 문구를 넣지 않습니다. 필요한 설명과 출처는 이미지 위 본문에 작성하고, `alt`에는 구체적인 대체 텍스트를 적습니다. 이미지에 `demo.href`를 지정하면 클릭 시 해당 링크를 새 탭으로 엽니다. 본문 중간에 이미지를 추가할 때는 일반 Markdown 이미지 문법을 사용합니다.
