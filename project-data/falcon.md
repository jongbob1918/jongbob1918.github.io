...
overview: >-
  공군 부사관으로 근무하며 활주로의 새·야생동물과 나사·쓰레기 같은 이물질이 항공기 운항에 큰 위험이 된다는 것을 경험했습니다. 위험요소를 찾아 제거하려면 사람이 계속 점검해야 하지만, 인력 부족과 관리 비용이 부담이었습니다. FALCON은 이 부담을 줄이고 위험을 제때 알리기 위해 고정형 폐쇄회로 카메라(CCTV) 영상을 인공지능(AI)으로 분석하는 운항안전 시스템입니다. 지상 위험요소를 탐지·추적해 관제 화면에 표시하고 조종사에게 음성 경보로 전달합니다.
demo:
  type: youtube
...
실제 공항 사진과 모형 영상은 객체가 보이는 크기와 배경이 달랐습니다. 이를 성능 저하의 원인 중 하나로 보고, 모형과 비슷한 장면을 학습에 추가하기 위해 합성 데이터를 만들었습니다.
