...
## 담당 범위 — Ground IDS와 좌표변환 검증

4명으로 구성된 팀의 팀장으로 일정과 문서를 관리하고 Ground IDS 통합을 담당했습니다. 실사·합성 데이터를 이용한 지상 객체 모델의 조사·학습·검증, YOLOv8과 ByteTrack을 연결한 추론 파이프라인, ArUco Marker 기반 좌표변환 시험을 수행했습니다. 합성 데이터 생성은 장진혁, 서버와 좌표변환 로직 설계는 박효진, 관제 GUI는 김지연과 협업했습니다.

## 데이터셋 개선 — 합성·실사·Negative Sample 결합
...
프레임마다 새 객체로 처리하지 않도록 YOLOv8의 추론 결과를 ByteTrack에 연결하고 `persist` 옵션으로 Track ID를 유지했습니다. IDS는 모델의 기본 클래스에 후처리를 더해 관제 판단에 필요한 상태를 구분합니다.
...
## 시스템 연동 — Map·Object Mode와 TCP 이벤트

IDS는 보정용 Map Mode와 위험요소 탐지용 Object Mode를 분리합니다. 추론 프로세스가 만든 객체 ID·클래스·좌표·신뢰도는 JSON 이벤트로 변환되어 TCP Queue를 통해 Main Server에 전달되고, 서버 명령으로 두 모드를 전환할 수 있습니다.
