...
overview: >-
  공군 부사관으로 근무하며 활주로의 새·야생동물과 나사·쓰레기 같은 이물질이 항공기 운항에 큰 위험이 된다는 것을 경험했습니다. 위험요소를 찾아 제거하려면 사람이 계속 점검해야 하지만, 인력 부족과 관리 비용이 부담이었습니다. 이 문제를 해결하기 위해 공항에 설치된 카메라 영상에서 위험요소를 인공지능으로 감지하고 관리자에게 알려주는 서비스를 개발했습니다.
demo:
  type: youtube
...
지상 탐지 대상은 조류, 이물질, 야생동물, 사람, 차량, 항공기 6종으로 정했습니다. 실제 공항 사진 중심의 공개 이미지 약 15,000장으로 첫 모델을 학습했지만, 공항 모형 영상에서는 탐지 정확도가 떨어졌습니다. 동물을 사람으로 잘못 분류하는 사례도 있었습니다.

모형을 직접 촬영한 이미지에 객체 위치를 라벨링해 학습 데이터를 추가했습니다. 하지만 프로젝트 기간과 인력이 한정돼 수작업으로는 충분한 양을 확보하기 어려웠습니다. 그래서 시뮬레이션에서 이미지와 라벨을 대량으로 생성하는 파이프라인을 구축했습니다.

## 합성·실제 데이터를 결합한 모델 개선

팀에서 Polycam과 Blender로 모형을 가상 환경에 재현했습니다.

<figure class="feature-media"><img src="../assets/images/falcon_simulated_airport_scene.png" alt="Polycam으로 스캔한 차량·조류·동물·이물질 모형과 Blender에 재현한 공항 모형 환경" loading="lazy"></figure>

합성 데이터 담당 팀원은 Unity로 촬영 각도·조명을 바꾸며 이미지와 객체 위치 라벨을 자동 생성했습니다.
...
합성 이미지 3,000장과 직접 촬영·라벨링한 이미지 1,000장을 섞어 경량 객체 탐지 모델 YOLOv8n을 학습했습니다.
...
마커 중심의 실제 위치를 측정하고 영상에서 같은 마커의 픽셀 위치를 찾았습니다. 두 좌표의 대응 관계로 평면 좌표변환 행렬(Homography)을 구해 객체의 중심점을 지도 좌표로 옮기고, 활주로·유도로·잔디 중 어느 구역에 있는지 판단했습니다.
