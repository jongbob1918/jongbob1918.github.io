...
## Robot State Machine — 상태와 임무 흐름 제어
...
## 팀 모듈 인터페이스 — State Machine과 하위 모듈 연결
...
<div class="evidence-grid"><div class="evidence-card"><span class="status-tag status-demo">DEMO</span><strong>협소 공간 자율주행</strong><p>Smac Hybrid와 MPPI를 적용한 후 60 cm 협로를 충돌 없이 통과하는 주행을 시연했습니다.</p></div><div class="evidence-card"><span class="status-tag status-confirmed">CODE</span><strong>상태 전이와 예외 처리</strong><p>완료·실패·취소와 배터리 조건에 따른 State Machine 전이를 Mock 응답으로 확인했습니다.</p></div><div class="evidence-card"><span class="status-tag status-confirmed">TOOL</span><strong>실물 로봇 모니터링</strong><p>현재 State, 세부 임무, 배터리와 ROS 로그를 모니터링 GUI에서 함께 확인했습니다.</p></div></div>

<div class="limitation"><strong>한계</strong> 반복 주행 성공률과 도서 요청부터 픽업까지의 전체 임무 시간은 별도로 측정하지 못했습니다.</div>

## Repository

[JAVIS DMC 구현과 테스트 보기 ↗](https://github.com/jongbob1918/JAVIS/tree/main/javis_ros2/src/javis_dmc)
