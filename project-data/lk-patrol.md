...
card:
  ...
  imageFit: cover
introduction: >-
  고양 서오릉에서 화재와 쓰러진 사람을 탐지하는 야외 순찰로봇의 자율주행 소프트웨어를 개발했습니다. 위치 추정·주행 제어·임무 수행·도킹 기능을 담당했습니다.
---

## 서오릉의 흙길과 경사로

서오릉의 2 km 순찰 경로는 대부분 흙길이며, 경사와 요철이 많습니다. 비가 오면 흙이 유실돼 길의 형태와 단차도 달라집니다. 사람이 함께 이용하는 공간이라 지형 대응과 사람 접근 시의 동작을 함께 고려해야 했습니다.
...
고정된 높이를 기준으로 장애물을 판별하는 기존 방식으로는 경사진 지면과 실제 장애물을 구분하기 어려웠습니다.
...
지도 작성과 동시에 위치를 추정하는 SLAM(Simultaneous Localization and Mapping), 지도에서 현재 위치를 찾는 Localization 기능에도 예외 처리를 추가하고 현장에 맞게 설정을 조정했습니다. 도킹 위치에 진입하고 이탈하는 도킹·언도킹 기능도 개발했습니다.
