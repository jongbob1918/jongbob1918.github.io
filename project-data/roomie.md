...
group: key
order: 2
title: ROOMIE — 저비용 로봇팔의 시각 피드백 제어
...
card:
  ...
  keywords:
    ...
    - Visual Feedback
    - FreeRTOS
  descriptionKo: 약 5만 원대 4축 로봇팔의 백래시와 2D 카메라의 깊이 한계를 반복 관측과 단계적 보정으로 다뤘습니다.
  descriptionEn: Compensated for 2D depth ambiguity and low-cost arm backlash through a repeat-observe-and-correct control loop.
overview: >-
  상용 로봇팔이나 Depth Camera 없이 엘리베이터 버튼을 조작하는 Arm Controller를 구현했습니다. 약 5만 원대 4축 로봇팔과 일반 2D 카메라를 사용했기 때문에 깊이 정보 부족, 서보모터 백래시, 프레임 변형과 토크 부족을 함께 다뤄야 했습니다. 버튼의 화면상 크기로 3차원 위치를 근사하고, 고정된 관측 자세와 반복 시각 피드백으로 위치 오차를 보정했습니다. 개발 당시 실제 버튼 활성화 성공률은 3/10이었으며, 위치 정렬과 물리적인 누름 힘을 서로 다른 문제로 분석했습니다.
overviewNote:
  title: 현재 구현의 검증 범위와 한계
  paragraphs:
...
<figure class="feature-media"><img src="../assets/images/roomie_elevator_flow.png" alt="엘리베이터 도착, 호출 버튼 조작, 탑승, 목적층 버튼 조작과 하차로 이어지는 ROOMIE 동작 흐름" loading="lazy"><figcaption>ROOMIE의 층간 이동 시나리오. 이 페이지는 김종명이 담당한 버튼 조작 Arm Controller에 집중합니다.</figcaption></figure>
...
<figure class="feature-media"><img src="../assets/images/roomie_system_architecture.png" alt="Guest, Staff, Admin GUI와 Roomie Main Server, 로봇 내부 제어 모듈을 연결한 ROOMIE 시스템 구성" loading="lazy"><figcaption>사용자 요청부터 로봇의 주행·비전·Arm Controller 실행까지 연결한 ROOMIE 시스템 아키텍처</figcaption></figure>
...
<div class="diagram" role="img" aria-label="Arm Controller 담당 범위"><div class="diagram-node">Vision Service<br>버튼 검출</div><div class="diagram-arrow">→</div><div class="diagram-node owner">3D 위치 추정<br>좌표계 변환</div><div class="diagram-arrow">→</div><div class="diagram-node owner">시각 피드백<br>4-DOF IK</div><div class="diagram-arrow">→</div><div class="diagram-node owner">ESP32 제어<br>ACK · 예외 처리</div><div class="diagram-arrow">→</div><div class="diagram-node">Action Result<br>Success · Abort</div></div>
...
<div class="evidence-grid"><div class="evidence-card"><span class="status-tag warning">PERCEPTION</span><strong>깊이 정보 부재</strong><p>2D 영상은 버튼의 픽셀 위치를 제공하지만 카메라와 버튼 사이의 거리를 직접 측정하지 못합니다.</p></div><div class="evidence-card"><span class="status-tag warning">MECHANISM</span><strong>백래시와 프레임 변형</strong><p>접근 방향과 관절 하중에 따라 같은 명령각에서도 로봇팔 끝단 위치가 달라졌습니다.</p></div><div class="evidence-card"><span class="status-tag warning">ACTUATION</span><strong>부족한 유효 토크</strong><p>팔을 펼친 상태에서는 버튼 반력을 이길 힘이 부족했고, 접촉 후 프레임이 밀리거나 휘었습니다.</p></div></div>
...
<div class="diagram" role="img" aria-label="관측 자세를 추가한 버튼 조작 순서"><div class="diagram-node">ClickButton<br>요청</div><div class="diagram-arrow">→</div><div class="diagram-node owner">OBSERVE_POSE<br>기준 자세 통일</div><div class="diagram-arrow">→</div><div class="diagram-node owner">0.5초 대기<br>버튼 재관측</div><div class="diagram-arrow">→</div><div class="diagram-node owner">위치 보정<br>Press · Retreat</div></div>
...
<figure class="feature-media"><img src="../assets/images/elevator-alignbutton.gif" alt="카메라로 버튼 위치를 반복 관측하며 로봇팔을 정렬하는 과정" loading="lazy"><figcaption>이동 후 버튼을 다시 관측하고 남은 위치 오차를 다음 이동에서 보정하는 과정</figcaption></figure>
...
<figure class="feature-media evidence-video"><video controls playsinline preload="metadata" poster="../assets/images/roomie-button-click-poster.jpg"><source src="../assets/videos/roomie-button-click.mp4" type="video/mp4">브라우저가 MP4 영상을 지원하지 않습니다.</video><figcaption>프로토타입 버튼 패널에서 수행한 접근·누르기·후퇴 동작 테스트. 동작 완료와 실제 버튼 활성화는 별도로 구분했습니다.</figcaption></figure>
...
<div class="diagram" role="img" aria-label="기하학 제어와 모방학습 잔여 보정을 결합한 향후 구조"><div class="diagram-node">2D Image<br>Joint Command</div><div class="diagram-arrow">→</div><div class="diagram-node">Geometry PBVS<br>Δp</div><div class="diagram-arrow">+</div><div class="diagram-node owner">Learned Residual<br>Δp<sub>learned</sub></div><div class="diagram-arrow">→</div><div class="diagram-node">IK · Safety Check<br>Arm Command</div></div>
...
<p class="note">저비용 하드웨어의 실패를 단순한 위치 오차로 숨기지 않고, 인식·제어·접촉력 문제로 분리해 분석했습니다.</p>
