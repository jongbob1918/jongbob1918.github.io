...
- **보상(Reward):** 행동의 결과로 에이전트가 받는 점수
- **환경(Environment):** 에이전트가 행동하고 그 결과와 보상을 받는 공간
- **상태(State):** 에이전트가 행동을 선택할 때 참고하는 현재 상황. 예: 현재 위치
- **행동 가치(Q값):** 현재 위치에서 특정 행동을 했을 때 앞으로 받을 보상의 예상값
- **정책(Policy):** 현재 위치에서 어느 방향으로 갈지 정하는 규칙. 예: Q값이 가장 높은 방향 선택

#### 맛집을 찾으며 배우는 에이전트

맛집까지 가는 길을 찾는 에이전트를 생각해 봅시다. 길을 찾는 주체가 **에이전트**, 지도와 점수 규칙이 **환경**입니다. 현재 위치는 **상태**, 위·아래·왼쪽·오른쪽 중 하나를 고르는 일은 **행동**입니다. 이동한 결과로 받는 점수가 **보상**입니다. 예를 들어 맛집에 도착하면 `+100`, 공사 구간에 들어가면 `−20`, 한 칸 이동할 때마다 `−1`을 받도록 정할 수 있습니다.
...
에이전트는 위치와 방향마다 앞으로 받을 보상의 예상 점수인 **Q값**을 기록합니다. 공사 구간으로 이동한 방향의 Q값은 낮추고, 맛집에 도착하는 데 도움이 된 방향의 Q값은 높입니다. 이렇게 배운 Q값을 보고 방향을 고르는 규칙이 **정책**입니다.
