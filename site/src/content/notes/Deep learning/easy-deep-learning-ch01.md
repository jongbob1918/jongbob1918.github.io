...
맛집까지 가는 길을 찾는 에이전트를 생각해 봅시다. 에이전트는 현재 위치에서 위·아래·왼쪽·오른쪽 중 하나를 선택하고, 이동한 결과에 따라 보상을 받습니다. 예를 들어 맛집에 도착하면 `+100`, 공사 구간에 들어가면 `−20`, 한 칸 이동할 때마다 `−1`을 받도록 환경을 설계할 수 있습니다.

학습 초반에는 어느 길이 좋은지 모르기 때문에 여러 방향으로 움직이며 시행착오를 겪습니다. 공사 구간으로 이동한 행동의 점수는 낮추고, 맛집에 도착하는 데 도움이 된 행동의 점수는 높입니다. 이 과정을 반복하면 에이전트는 각 위치에서 보상이 큰 행동을 선택하게 됩니다.

하지만 지금까지 가장 높은 보상을 준 길만 계속 선택하면, 다른 위치에 있는 더 좋은 맛집을 발견하지 못할 수 있습니다. 이를 해결하려면 이미 알고 있는 행동을 선택하는 **활용(Exploitation)** 과 새로운 행동을 시도하는 **탐색(Exploration)** 사이의 균형이 필요합니다.
...
<img src="/images/notes/easy-deep-learning-ch01/restaurant-reinforcement-learning.png" alt="왼쪽은 엡실론 그리디로 기존 맛집 경로를 활용하면서 무작위 탐색으로 보상이 더 큰 맛집을 발견하는 과정이고, 오른쪽은 이동 벌점과 할인율로 긴 경로보다 짧은 경로의 누적 보상이 커지는 과정" loading="lazy" />
