...
사람이 모든 판단 규칙을 직접 정하는 대신, 데이터에서 판단에 필요한 패턴을 학습하는 방법입니다.
...
5×5 지도에서 맛집을 찾는 로봇을 생각해 봅시다. 로봇은 행동을 선택하는 **에이전트(Agent)**이고, 지도와 보상 규칙은 **환경(Environment)**입니다. 로봇이 있는 칸은 **상태(State)**, 위·아래·왼쪽·오른쪽으로 한 칸 움직이는 것은 **행동(Action)**입니다.

맛집에 도착하면 `+100`, 공사 구간에 들어가면 `−20`을 받습니다. 어느 방향이든 한 칸 이동할 때마다 `−1`도 받습니다.

처음에는 어느 길이 좋은지 모르므로 여러 방향을 무작위로 시도합니다. 어느 순간 공사 구간에 들어가 벌점을 받았습니다.

<img src="../../../../public/images/notes/easy-deep-learning-ch01/restaurant-rl-01-random-exploration.png" alt="로봇 에이전트가 사각판을 탐색하다 공사 구간에 들어가는 점선 경로. 이동할 때마다 1점, 공사 구간 진입 시 20점을 잃는다" loading="lazy" />

여러 번 시도하면서 로봇은 공사 구간을 피하고 맛집에 도달하는 길을 찾습니다. 한 칸 움직일 때마다 `−1`을 받으므로, 같은 맛집이라면 적은 칸을 이동하는 길이 유리합니다.

<img src="../../../../public/images/notes/easy-deep-learning-ch01/restaurant-rl-02-reward-update.png" alt="로봇 에이전트가 공사 구간을 피해 맛집에 도착하는 파란 경로. 한 칸 이동할 때마다 1점을 잃고 맛집에서 100점을 받는다" loading="lazy" />

현재 칸에서 어느 방향으로 갈지 정하는 규칙을 **정책(Policy)**이라고 합니다. 이런 학습에 깊은 신경망을 사용하면 **심층 강화학습(Deep Reinforcement Learning)**이 됩니다.
