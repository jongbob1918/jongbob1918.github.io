...
**바이어스** 는 입력노드에 엣지로부터 오는 값들의 총합에 더해지는 값으로, 신경의 민감도를 조절하는 파라미터이다.
...
예를 들어 다음과 같은 두 영상이 있다고 하자. 표에서 조회수는 **만 회**, 수익은 **만 원** 단위다.
...
조회수와 예측 수익의 관계를 식으로 나타내면 다음과 같다.
...
목표는 실제 수익을 잘 예측하는 $a, b$를 찾는 것이다. 그러려면 어떤 값이 더 나은지 판단할 기준이 필요하다.

### 예측을 평가하는 손실 함수

**손실 함수**(Loss Function)는 예측과 실제 수익의 차이를 하나의 수치로 나타낸다.
...
오차가 2에서 10으로 커지면 절댓값은 5배, 제곱값은 25배가 된다. 어떤 영상에서 예측 오차가 크게 발생하면 MSE는 그 오차를 줄이는 데 더 큰 비중을 둔다.

이후 계산에서는 MSE를 기준으로 삼는다. 이제 이 값을 줄이도록 $a, b$를 바꾸는 방법을 살펴보자.
