---
title: "역전파 계산을 행렬로 따라가기"
slug: deep-learning/easy-deep-learning-ch03
description: 조회수와 영상 길이로 수익을 예측하는 작은 MLP를 만들고, 순전파에서 가중치 갱신까지 계산을 직접 따라갑니다.
...
역전파를 `미분을 뒤로 전파한다`라고만 외우면 행렬의 크기가 바뀌는 지점에서 계산을 놓치기 쉽다. 조회수와 영상 길이로 수익을 예측하는 작은 다층 퍼셉트론을 만들고, 각 값의 크기와 미분이 어떻게 이어지는지 확인해 본다.

## 노드의 계산을 행렬로 묶기

### 노드마다 계산하면 식이 길어진다

조회수와 영상 길이로 수익을 예측한다고 해보자. 다음 구조의 **다층 퍼셉트론**(Multi-Layer Perceptron, MLP)을 사용한다.
...
- 혁펜하임, 『이지 딥러닝』. MLP와 역전파의 기본 개념을 이해하는 데 참고했다.
