...
인공 신경망의 구조와 선형 회귀의 학습 과정. 조회수로 영상 수익을 예측하는 예시를 통해, 가중치와 바이어스를 어떻게 조정하는지 살펴본다.
...
| 생물학적 구조 | 역할 | 인공 신경에서 대응하는 계산 |
| --- | --- | --- |
| 수상돌기 | 다른 신경의 신호를 받음 | 연결을 통해 입력을 받음 |
| 세포체 | 받은 신호를 통합 | 가중합을 계산하고 활성화 함수를 적용 |
| 축삭 | 신호를 다음 신경으로 전달 | 출력을 다음 층으로 전달 |

**노드**는 계산을 수행하는 단위, **엣지·커넥션**은 노드 사이의 연결이다. 이런 인공 신경들을 연결한 것이 인공 신경망이다.

### 가중치와 바이어스

영상 수익을 예측할 때 조회수와 영상 길이를 입력한다고 해보자. 두 값이 수익에 미치는 영향은 다를 수 있다. 입력마다 **가중치**(Weight)를 곱하면 각 입력의 영향력을 다르게 조절할 수 있다.

**바이어스**(Bias)는 가중합에 더하는 값이다. 입력의 영향과 별개로, 활성화 함수에 들어가는 값의 기준을 옮긴다.
...
활성화 함수는 가중합을 다음 출력으로 바꾼다. 비선형 활성화 함수를 사용하면 신경망이 직선만으로 표현하기 어려운 관계도 학습할 수 있다.
...
1. 신경망은 가중치와 바이어스를 가진 함수다.
2. 손실 함수로 예측을 평가하고, 그래디언트로 파라미터를 갱신한다.
3. 배치 크기는 갱신에 쓸 데이터 양, 모멘텀·RMSprop·Adam은 이동 방식을 정한다.
4. 검증 데이터로 새 데이터에서도 성능이 좋아지는지 확인한다.
